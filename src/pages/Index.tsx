    ...
    <div className="min-h-screen bg-gradient-hero relative overflow-hidden">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 py-20 text-center z-10">
          <Badge className="mb-6 bg-gradient-cyber text-white border-none px-6 py-2 text-sm font-orbitron hover-glow animate-fade-in">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold mb-6 px-4 font-orbitron">
            <span className="bg-gradient-neon bg-clip-text text-transparent text-glow animate-fade-in">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12 px-4">
            <Button 
              variant="hero" 
              size="lg" 
              className="text-lg px-8 font-orbitron hover-glow bg-gradient-cyber border-none shadow-glow-cyber"
            ...
            <Button 
              variant="neon" 
              size="lg" 
              className="text-lg px-8 font-orbitron bg-gradient-neon border-none shadow-neon" 
              asChild
            >
              <Link to="/apply">
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gaming-dark/80 backdrop-blur-sm relative z-10">
        <div className="container mx-auto px-4">
          <div className="text-center mb-8 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold bg-gradient-neon bg-clip-text text-transparent mb-4 font-orbitron">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 auto-rows-fr justify-items-center place-content-center">
            {homepageFeatures.map((feature, index) => (
              <Card 
                key={index} 
                className="group p-6 bg-gradient-card backdrop-blur-sm border-gaming-border hover:border-cyber-purple/70 transition-all duration-500 hover:shadow-glow-cyber hover:scale-105 animate-fade-in flex flex-col h-full floating"
                style={{ animationDelay: `${index * 200}ms` }}
              >
                <div className="flex flex-col items-center text-center flex-grow">
                  <div className="mb-4 p-4 rounded-full bg-gradient-cyber/20 group-hover:bg-gradient-cyber/40 transition-all duration-300 border border-cyber-purple/30">
                    <DynamicIcon name={feature.icon} className={`h-8 w-8 ${feature.color} group-hover:scale-110 transition-transform duration-300`} />
                  </div>
                  <h3 className="text-xl font-semibold text-foreground mb-3 group-hover:text-cyber-purple transition-colors duration-300 font-orbitron">{feature.title}</h3>
                ...
                {homepageCta.features?.map((feature: string, index: number) => (
                  <div key={index} className="flex items-center space-x-3 group">
                    <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
                      index % 3 === 0 ? 'bg-neon-green shadow-[0_0_10px_hsl(var(--neon-green))]' : 
                      index % 3 === 1 ? 'bg-neon-blue shadow-[0_0_10px_hsl(var(--neon-blue))]' : 'bg-neon-purple shadow-[0_0_10px_hsl(var(--neon-purple))]'
                    } group-hover:scale-125`} />
                    <span className="text-foreground font-inter group-hover:text-cyber-purple transition-colors">{feature}</span>
                  </div>
                ))}
              </div>
            </div>
            
            <Card className="p-8 bg-gradient-card backdrop-blur-sm border-cyber-purple/30 shadow-glow-cyber hover-glow">
              <div className="text-center">
                <h3 className="text-2xl font-bold text-foreground mb-6 font-orbitron flex items-center justify-center">
                  <Gamepad2 className="h-6 w-6 mr-2 text-cyber-purple" />
                  SERVER INFORMATION
                </h3>
                <div className="space-y-6">
                  <div className="flex justify-between items-center p-3 bg-gaming-darker/50 rounded-lg border border-gaming-border/50">
                    ...
                    <code className="bg-gaming-dark px-4 py-2 rounded font-mono text-cyber-purple border border-cyber-purple/30">
                  ...
                  <div className="flex justify-between items-center p-3 bg-gaming-darker/50 rounded-lg border border-gaming-border/50">
                    ...
                    <Button 
                      variant="neon" 
                      size="sm"
                      className="font-orbitron bg-gradient-cyber border-none"
