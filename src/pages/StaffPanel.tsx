...
const StaffPanel = () => {
  ...
  const [selectedApplicationView, setSelectedApplicationView] = useState('pending');
  ...
  const syncUserDiscordRoles = async (userId: string) => {
    try {
      if (!serverSettings.discord_settings?.server_id || !serverSettings.discord_settings?.auto_roles) {
        return;
      }

      // Get user's internal roles
      ...
      // Get user's Discord ID (you'll need to collect this during application process)
      ...
      if (profileError || !profile?.discord_id) {
        console.log('User discord_id not found, skipping role sync');
        return;
      }

      const internalRoles = userRoles.map(r => r.role);
      ...
      if (data.success) {
        console.log('Discord roles synced successfully:', data.data);
      } else {
        throw new Error(data.error);
      }
    } catch (error) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="p-6 bg-gaming-card border-gaming-border shadow-gaming">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <Label className="text-foreground">Approved Role ID (Legacy)</Label>
                    <Input
