...
-- Insert default staff roles only if they don't exist
...
SELECT * FROM (VALUES
  ...
  ('Helper', '#48CAE4', 40),
  ('Staff', '#A8E6CF', 50)
) AS v(display_name, color, hierarchy_level)
WHERE NOT EXISTS (
  SELECT 1 FROM public.staff_roles WHERE staff_roles.display_name = v.display_name
);
