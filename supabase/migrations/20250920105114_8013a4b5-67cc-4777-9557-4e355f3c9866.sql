-- Migrate to custom roles system only (fixed UUID casting)
...
VALUES 
  ('11111111-1111-1111-1111-111111111111'::uuid, 'super_admin', 'Super Administrator', 'Full system access with all permissions', '#7c3aed', 100, true),
  ('22222222-2222-2222-2222-222222222222'::uuid, 'admin', 'Administrator', 'Full access to most features', '#dc2626', 90, true),
  ('33333333-3333-3333-3333-333333333333'::uuid, 'staff', 'Staff Member', 'Access to management features', '#2563eb', 70, true),
  ('44444444-4444-4444-4444-444444444444'::uuid, 'moderator', 'Moderator', 'Basic moderation permissions', '#059669', 50, true)
...
SELECT 
  cu.id,
  CASE 
    WHEN cu.role = 'admin' THEN '22222222-2222-2222-2222-222222222222'::uuid
    WHEN cu.role = 'staff' THEN '33333333-3333-3333-3333-333333333333'::uuid
    WHEN cu.role = 'moderator' THEN '44444444-4444-4444-4444-444444444444'::uuid
    ELSE NULL
  END as role_id,
  NOW(),
  true
